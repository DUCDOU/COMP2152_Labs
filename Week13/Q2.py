...
def severity_summary(findings):
    severity_counts = count_by_field(findings, "severity")
    return [(sev, severity_counts.get(sev, 0)) for sev in ["HIGH", "MEDIUM", "LOW"]]
...
def timeline(findings):
    date_counts = count_by_field(findings, "date")
